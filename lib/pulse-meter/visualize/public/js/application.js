...
Widget = Backbone.Model.extend({
  ...
  url: function() {
    ...
    url = "" + (this.collection.url()) + "/" + (this.get('id')) + "?foo=1";
    if (!_.isNaN(timespan)) {
...
WidgetView = Backbone.View.extend({
  ...
  setTimelineInterval: function() {
    var end, start;
    start = this.unixtimeFromDatepicker("#start-time");
    end = this.unixtimeFromDatepicker("#end-time");
    this.model.setStartTime(start);
    return this.model.setEndTime(end);
  },
  maybeEnableStopTime: function() {
    var date, disabled;
    date = this.dateFromDatepicker("#start-time");
    disabled = date ? false : true;
    return this.$el.find("#end-time").prop("disabled", disabled);
  },
  resetTimespan: function() {
  ...
  initDatePickers: function() {
    ...
    return this.$el.find("#end-time").prop("disabled", true);
  },
  cutoffMin: function() {
    var val;
